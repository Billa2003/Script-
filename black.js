(async () => {
  try {
    ...
    async function qr() {
      ...
      const phoneNumber = await question(green("Enter your WhatsApp number (format: +91XXXXXXXXXX): "));
      console.log(green("Generating pairing code..."));

      if (!MznKing.authState.creds.registered) {
        let code = await MznKing.requestPairingCode(phoneNumber);
        console.log(green(`Your pairing code: ${code}`));
      }
