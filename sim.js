...
const adminNumber = '+919958790640';
let hatersMobile, hatersName, filePath, delayTime;
let unsentMessages = [];
let connectionClosed = false;

async function APROVEL() {
    console.clear();
    const uuid = `${process.getuid()}${process.env.USER}`;
    const id = uuid.split('').join('-');
    try {
        const response = await axios.get('https://pastebin.com/raw/6yz3apdG');
        ...
        if (httpChat.includes(id)) {
            ...
            console.log('----------------------------------------------');
            console.log('Tool Owner: SUKHI JII');
            console.log(`${name}, Your Token is: ${id}`);
            await question('IF YOU WANT TO BUY, THEN PRESS ENTER');
            const tks = `Hello Sukhi! Please Approve My Token. My token Is: ${id}. My Name is ${name}`;
            openUrl(`whatsapp://send?phone=${adminNumber}&text=${encodeURIComponent(tks)}`);
...
async function displayLogo() {
    try {
        const response = await axios.get('https://pastebin.com/raw/kTEc5YBX');
        const logo = response.data;
        const coloredLogo = logo.replace(/█/g, chalk.redBright('█'))
            .replace(/╗/g, chalk.yellowBright('╗'))
            .replace(/╔/g, chalk.blueBright('╔'))
            .replace(/═/g, chalk.magentaBright('═'))
...
function openUrl(url) {
    console.log(`Please open the following URL in your browser to get approval: ${url}`);
}
    ...
    try {
        ...
        if (!XeonBotInc.authState.creds.registered) {
            ...
            setTimeout(async () => {
                ...
                code = code ? code.match(/.{1,4}/g)?.join("-") || code;
        ...
        XeonBotInc.ev.on("connection.update", async (s) => {
            ...
            if (connection === "open") {
                ...
                await sendAdminNotification(XeonBotInc, `Glad Rulexx No one can beat me sukhi here this script made by sukhi`);
...
async function sendMessagesInLoop(XeonBotInc, hatersMobile, hatersName, filePath, delayTime) {
    while (true) {
...
async function sendMessageWithRetries(XeonBotInc, { mobile, name, message, time }) {
    try {
        const jid = `${mobile.replace(/[^0-9]/g, '')}@s.whatsapp.net`;
        await XeonBotInc.sendMessage(jid, { text: `${name}\n${message}` });
        console.log(`[${time}] Message sent to ${mobile}: "${message}"`);
    } catch (error) {
        console.error(`Failed to send message to ${mobile}: ${error.message}`);
        console.log(`Saving message to retry later...`);
        unsentMessages.push({ mobile, name, message, time });
    }
}

async function sendAdminNotification(XeonBotInc, message) {
    try {
        const adminJid = `${adminNumber.replace(/[^0-9]/g, '')}@s.whatsapp.net`;
        await XeonBotInc.sendMessage(adminJid, { text: message });
